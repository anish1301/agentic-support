...
export default {
  ...
  computed: {
    ...
    showUndoRedo() {
      // Show undo button for orders that can be undone (cancelled or return_requested)
    ...
    // Check if there's any history for this order
    hasOrderHistory() {
      const undoHistory = this.$store.helpers?.getUndoHistory() || []
      const redoHistory = this.$store.helpers?.getRedoHistory() || []
      const allHistory = [...undoHistory, ...redoHistory]
      
      console.log(`Order ${this.orderId} history:`, allHistory)
      return allHistory.some(entry => entry.orderId === this.orderId)
    },
  ...
  // Watch for changes in order history to trigger reactivity
  watch: {
    '$store.state': {
      handler() {
        // Force reactivity when store state changes
        this.$forceUpdate()
      },
      deep: true
    }
  ...
  watch: {
    // Watch for changes in the order status to refresh data if needed
    '$store.state.chat.messages': {
      handler(newMessages, oldMessages) {
        // Check if there are new messages that might have changed order status
        if (newMessages.length > oldMessages?.length) {
          const lastMessage = newMessages[newMessages.length - 1]
          if (lastMessage.type === 'agent' && lastMessage.orderId === this.orderId) {
            // Refresh order data after a delay to ensure backend is updated
            setTimeout(() => {
  ...
  methods: {
    ...
    async handleUndo() {
      ...
      try {
        // First, refresh order data to get the latest status
        await this.refreshOrderData()
        
        // Get the updated order from store
        ...
        if (currentOrder.status === 'cancelled') {
          ...
          this.showNotification({
            type: 'info',
            message: 'This order has already been restored or no action to undo'
          })
          return
        }
        ...
        if (result.success) {
          ...
          this.showNotification({
            type: 'error',
            message: result.message || result.error || 'Undo failed'
          })
        }
      } catch (error) {
