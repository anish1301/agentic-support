...
class MongoChatService {
  constructor() {
    this.baseUrl = 'http://localhost:3001' // Your server URL
  ...
  async getRecentSessions(limit = 10) {
    try {
      const response = await fetch(`${this.baseUrl}/api/chat/sessions?limit=${limit}`)
      const result = await response.json()
  ...
  async getChatAnalytics() {
    try {
      const response = await fetch(`${this.baseUrl}/api/chat/analytics`)
      const result = await response.json()
